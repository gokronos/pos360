...
export async function GET(req: Request) {
  ...
  const rows = await d
    .prepare(
      "SELECT id,sku,barcode,name,category,price,cost,stock,version,active,updated_at updatedAt FROM products WHERE tenant_id=? AND (name LIKE ? OR sku LIKE ? OR barcode LIKE ?) ORDER BY active DESC,name LIMIT 200",
    )
...
export async function POST(req: Request) {
  ...
  try {
    await d.batch([
      d
        .prepare(
          "INSERT INTO products (id,tenant_id,sku,barcode,name,category,price,cost,stock) VALUES (?,?,?,?,?,?,?,?,?)",
        )
        .bind(
          id,
          T,
    ...
    return Response.json(
      { product: { id, ...p, stock, active: 1, version: 1 } },
      { status: 201 },
    );
  } catch (e) {
    return Response.json(
...
export async function PATCH(req: Request) {
  ...
  const updated = await d
    .prepare(
      "SELECT id,sku,barcode,name,category,price,cost,stock,version,active,updated_at updatedAt FROM products WHERE id=?",
    )
    .bind(p.id)
    .first();
